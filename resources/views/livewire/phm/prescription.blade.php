<div>
                ...
                <tr wire:key="line-{{ $t['id'] }}"
                    ...
                    <td>{{ number_format(TreatmentService::getPrice($t['item_id'], $t['profile']) ?? 0) }}
                        ...
                        <select class="form-control" wire:model="prescriptions.{{ $i }}.profile"
                            wire:change="updateProfile({{ $i }}, $event.target.value)" @readonly($t['status'] == Status::completed)>
                    ...
                    <td>
                        <input type="number" wire:change="compute"
                            ...
                            @readonly($t['status'] == Status::completed) />
                    </td>
                    <td>
                        {{ config('app.currency') }}
    ...
    <x-modal id="dispense-confirm">
        ...
        <div class="py-4">
            <button wire:click="confirmDispense" class="btn bg-red-400 text-white">Confirm <i class="fa fa-save"></i></button>
            <button wire:click="confirmDispense(true)" class="btn bg-red-500 text-white">Confirm & Close <i class="fa fa-save"></i></button>
        </div>
    </x-modal>
</div>
